...
class RateLogger(RateMeter):
    ...
    def text_log(self, cur_iter, rate, lr):
        if lr > 1e-13:
            self.logger.info('Train Epoch: {:3d} Rate: {:.2f}  (lr: {:.8f})'.format(cur_iter, rate, lr))
        elif lr < -1e-13:
            self.logger.info('           Test Epoch: {:3d} Rate: {:.2f} '.format(cur_iter, rate))
        else:
            self.logger.info('          Valid Epoch: {:3d} Rate: {:.2f} '.format(cur_iter, rate))
...
class RDLogger(RateDistortionMeter):
    def __init__(self, distortion='mse'):
    ...
    def text_log(self, cur_iter, loss, mse, rate, rate2, lr, typ):
        if self.distortion == 'mse':
            ...
            dist_text = 'MSE/PSNR'
        elif self.distortion == 'ms_ssim' or self.distortion == 'ssim':
            psnr = -10*torch.log10(torch.Tensor([mse])).item()  # mse here is actually  1-ms_ssim or 1-ssim
            if self.distortion == 'ms_ssim':
                dist_text = 'MSSSIM/dB'
            elif self.distortion == 'ssim':
                dist_text = 'SSIM/dB'
        ...
        if rate2 < 10**-6:
            ...
            if typ == 'tr':
                self.logger.info('  Train Epoch: {:3d}  RDLoss: {:.6f} {}: {:.6f}/{:.2f} Rate: {:.3f}  (lr: {:.6f}) ({})'.format(cur_iter, loss, dist_text, mse, psnr, rate, lr, timedifstr))
            elif typ == 'te':
                self.logger.info('   Test Epoch: {:3d}  RDLoss: {:.6f} {}: {:.6f}/{:.2f} Rate: {:.3f} ({})'.format(cur_iter, loss, dist_text, mse, psnr, rate, timedifstr))
            elif typ == 'va':
                self.logger.info('  Valid Epoch: {:3d}  RDLoss: {:.6f} {}: {:.6f}/{:.2f} Rate: {:.3f} ({})'.format(cur_iter, loss, dist_text, mse, psnr, rate, timedifstr))
            elif typ == 'it':
                self.logger.info('Train Itera: {:3d}  RDLoss: {:.6f} {}: {:.6f}/{:.2f} Rate: {:.3f}  (lr: {:.6f}) ({})'.format(cur_iter, loss, dist_text, mse, psnr, rate, lr, timedifstr))
        else:
            if typ == 'tr':
                self.logger.info('  Train Epoch: {:3d}  RDLoss: {:.6f} {}: {:.6f}/{:.2f} Rate: {:.3f}+{:.3f}  (lr: {:.6f}) ({})'.format(cur_iter, loss, dist_text, mse, psnr, rate, rate2, lr, timedifstr))
            elif typ == 'te':
                self.logger.info('   Test Epoch: {:3d}  RDLoss: {:.6f} {}: {:.6f}/{:.2f} Rate: {:.3f}+{:.3f} ({})'.format(cur_iter, loss, dist_text, mse, psnr, rate, rate2, timedifstr))
            elif typ == 'va':
                self.logger.info('  Valid Epoch: {:3d}  RDLoss: {:.6f} {}: {:.6f}/{:.2f} Rate: {:.3f}+{:.3f} ({})'.format(cur_iter, loss, dist_text, mse, psnr, rate, rate2, timedifstr))
            elif typ == 'it':
                self.logger.info('Train Itera: {:3d}  RDLoss: {:.6f} {}: {:.6f}/{:.2f} Rate: {:.3f}+{:.3f}  (lr: {:.6f}) ({})'.format(cur_iter, loss, dist_text, mse, psnr, rate, rate2, lr, timedifstr))
